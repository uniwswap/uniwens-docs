...
The controller works exclusively with plaintext labels \(eg, 'alice' for 'alice.ethw'\).
...
`makeCommitment` generates and returns a commitment hash from a name label \(eg, 'myname', not 'myname.ethw'\) owner, and secret value.
...
A successful call also results in the Registrar emitting a [Name Registered Event](registrar.md#name-registered), and the POWNS registry emitting a [New Owner Event](../dns.md#set-subdomain-owner).
