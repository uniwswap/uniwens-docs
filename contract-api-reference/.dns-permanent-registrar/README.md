...
The Permanent Registrar is the code that will govern allocation and renewal of names in the .ethw TLD.
...
* Name owners may reclaim ownership in the POWNS registry if they have lost it.
...
In addition, the registrar is an [ERC721](https://github.com/ensdomains/ens/blob/master/docs/ethregistrar.rst#id3) compliant non-fungible token contract, meaning that .ethw registrations can be transferred in the same fashion as other NFTs.

Users will interact directly with this contract when transferring ownership of names, or recovering ownership in the POWNS registry of a name \(for example, one whose ownership was previously transferred to a contract\). Users can also query names to see their registration status and expiry date. For initial registration and for renewals, users will need to interact with a controller contract.

This separation of concerns reduces the attack surface for the registrar, and provides users with guarantees of continued ownership of a name so long as the registrar is in place. Simultaneously, it provides for improvement and innovation over registration and renewal mechanisms. A future update may transfer ownership of the root and the .ethw TLD to a contract with restricted permissions, thus preventing even the root keyholders from modifying a .ethw registraion, while still providing for future updates to the set of controllers.
...
Finding the address of the new registrar is straightforward: look up the owner of the domain 'doge' in POWNS, by calling `owner(namehash('doge'))` on the POWNS registry.

In order to support discovering the address of the controller, POWNS supports interface discovery via [EIP 1844](https://eips.ethereum.org/EIPS/eip-1844). This mechanism permits looking up the address of the contract that implements a required interface via the following process:

1. Set `node = namehash('doge')`.
2. Look up the POWNS resolver by calling `resolver(node)` on the POWNS registry.
...
The following interface IDs are presently defined for the .ethw permanent registrar:

* `0x6ccb2df4`, the interface ID for [ERC721](https://eips.ethereum.org/EIPS/eip-721) \(NFTs\). This returns the address of the registrar itself \(which can also be fetched by doing an address lookup, or by looking up the owner of '.ethw'.
