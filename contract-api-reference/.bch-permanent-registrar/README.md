...
The Permanent Registrar is the code that will govern allocation and renewal of names in the .bch TLD.

## System architecture

Code for the permanent registrar can be found in the [bchregistrar](https://github.com/bchdomains/bchregistrar) repository.
...
In addition, the registrar is an [ERC721](https://github.com/ensdomains/ens/blob/master/docs/ethregistrar.rst#id3) compliant nonfungable token contract, meaning that .bch registrations can be transferred in the same fashion as other NFTs.

Users will interact directly with this contract when transferring ownership of names, or recovering ownership in the LNS registry of a name \(for example, one whose ownership was previously transferred to a contract\). Users can also query names to see their registration status and expiry date. For initial registration and for renewals, users will need to interact with a controller contract.
...
Finding the address of the new registrar is straightforward: look up the owner of the domain 'bch' in LNS, by calling `owner(namehash('bch'))` on the LNS registry.

In order to support discovering the address of the controller, LNS supports interface discovery via [EIP 1844](https://eips.ethereum.org/EIPS/eip-1844). This mechanism permits looking up the address of the contract that implements a required interface via the following process:

1. Set `node = namehash('bch')`.
2. Look up the LNS resolver by calling `resolver(node)` on the LNS registry.
