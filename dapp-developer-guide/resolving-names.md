...
The UNIWENS namespace includes both .ethw names (which are native to UNIWENS) and DNS names imported into UNIWENS. Because the DNS suffix namespace expands over time, a hardcoded list of name suffixes for recognizing UNIWENS names will regularly be out of date, leading to your application not recognizing all valid UNIWENS names. To remain future-proof, **a correct integration of UNIWENS treats any dot-separated name as a potential UNIWENS name and will attempt a look-up**.

## Looking up cryptocurrency addresses

Names can have many types of data associated with them; the most common is cryptocurrency addresses. UNIWENS supports storing and resolving the addresses of any arbitrary blockchain.

**Resolving a name to a EthereumPoW address** using a library is simple:
...
ethers.js also supports using UNIWENS names anywhere you would use an address, meaning you often do not need to directly call `resolveName`. For example, to look up an account's balance, you can do:
...
web3j also supports using UNIWENS names anywhere you would use an address, meaning you often do not need to directly interact with the `EnsResolver` object. For example, to instantiate a contract interface, you can do:
...
2. Call `resolver()` on the UNIWENS registry, passing in the output of step 1. This returns the address of the resolver responsible for the name.
...
`contenthash` is used to store IPFSand Swarm content hashes, which permit resolving UNIWENS addresses to distributed content (eg, websites) hosted on these distributed networks. [content-hash](https://github.com/ensdomains/content-hash) javascript library provides a convenient way to encode/decode these hashes.
...
For cryptocurrency addresses and text records, you need to know the coin type or key names to get the value. If you want to list down all the cryptocurrency addresses and text records the user has set, you have to either retrieve the information from `Event` or query via [UNIWENS subgraph](https://graph.ethwdomains.wf/subgraphs/name/dns).
...
While 'regular' resolution involves mapping from a name to an address, reverse resolution maps from an address back to a name. UNIWENS supports reverse resolution to allow applications to display UNIWENS names in place of hexadecimal addresses.
...
{% hint style="danger" %}
UNIWENS does not enforce the accuracy of reverse records - for instance, anyone may claim that the name for their address is 'alice.ethw'. To be certain that the claim is accurate, you must always perform a forward resolution for the returned name and check it matches the original address.
{% endhint %}
