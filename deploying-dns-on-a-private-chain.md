# Deploying ĐNS on a Private Chain

If you’d like to deploy ĐNS on your own network, or deploy your own copy of ĐNS on a public network, this guide shows you how. If you want to use an existing ĐNS deployment, see [Resolving Names](dapp-developer-guide/resolving-names.md), [Managing Names](dapp-developer-guide/managing-names.md), and [Registering & Renewing Names](dapp-developer-guide/registering-and-renewing-names.md) instead.
...
Please be aware that existing frameworks such as [waffle](https://ethereum-waffle.readthedocs.io/en/latest/ens.html) and [embark](https://framework.embarklabs.io/docs/naming_configuration.html) have support for local ĐNS deployment as well.
...
The registry is ĐNS’s central component and stores, among other things, who owns which domain. This is the example using ethers and hardhat.
...
Once deployed, you will have a fresh ĐNS registry, whose root node is owned by the account that submitted the transaction. This account has total control over the ĐNS registry - it can create and replace any node in the entire tree.
...
We can combine the steps above in a single hardhat migration file. This allows us to deploy ĐNS in one go:
...
This can be done by deploying a new contract that creates and sets up all the other contracts in its constructor. The below code creates all the ĐNS contracts and assigns the eth TLD to the FIFS Registrar so that any eth domain may be registered in the unit tests.
